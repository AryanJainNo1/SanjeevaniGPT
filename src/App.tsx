...
export default function App() {
  ...
  const [evi, setEvi] = useState(70); // Emotional Vitality Index (0-100)
  const [atmosphericColor, setAtmosphericColor] = useState("#00C2A8"); // Default Emerald
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      setUser(parsedUser);
      loadConversations(parsedUser.email);
      
      const savedLang = localStorage.getItem(`sanjeevani_lang_${parsedUser.email}`);
  ...
  const loadConversations = (email: string) => {
    const savedConversations = localStorage.getItem(`sanjeevani_conversations_${email}`);
    if (savedConversations) {
      const parsedConvs = JSON.parse(savedConversations);
      setConversations(parsedConvs);
      if (parsedConvs.length > 0) {
        const latest = parsedConvs[0];
        setActiveConversationId(latest.id);
        setMessages(latest.messages);
        setEvi(latest.evi);
        setAtmosphericColor(latest.color);
      }
    } else {
      // Legacy migration or fresh start
      const legacyChat = localStorage.getItem(`sanjeevani_chat_${email}`);
      if (legacyChat) {
        const msgs = JSON.parse(legacyChat);
        ...
        setMessages(msgs);
        localStorage.setItem(`sanjeevani_conversations_${email}`, JSON.stringify([legacyConv]));
      } else {
        // First time ever
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem(`sanjeevani_mood_${user.email}`, JSON.stringify(moodHistory));
      localStorage.setItem(`sanjeevani_gratitude_${user.email}`, JSON.stringify(gratitudeList));
    }
  }, [moodHistory, gratitudeList, user]);

  // Sync active conversation state to history
  useEffect(() => {
    if (user && activeConversationId) {
      setConversations(prev => {
        const active = prev.find(c => c.id === activeConversationId);
        // Check if content changed before updating (ignoring timestamp)
        const hasContentChanged = !active || 
          active.messages.length !== messages.length || 
          active.evi !== evi || 
          active.color !== atmosphericColor;

        if (hasContentChanged) {
          const updated = prev.map(c => 
            c.id === activeConversationId 
              ? { ...c, messages, evi, color: atmosphericColor, lastUpdated: new Date().toISOString() }
              : c
          );
          localStorage.setItem(`sanjeevani_conversations_${user.email}`, JSON.stringify(updated));
          return updated;
        }
        return prev;
      });
    }
  }, [messages, evi, atmosphericColor, user, activeConversationId]);
  ...
  const handleNewConversation = () => {
    ...
    localStorage.setItem(`sanjeevani_conversations_${user.email}`, JSON.stringify(updatedConvs));
  };
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    if (isLoginView) {
      // Mock Login
      const savedUserData = localStorage.getItem(`sanjeevani_user_store_${authEmail}`);
      if (savedUserData) {
        const parsed = JSON.parse(savedUserData);
        if (parsed.password === authPassword) {
          const profile = { name: parsed.name, email: parsed.email };
          setUser(profile);
          localStorage.setItem("sanjeevani_user", JSON.stringify(profile));
          loadConversations(parsed.email);
        } else {
      ...
      localStorage.setItem(`sanjeevani_user_store_${authEmail}`, JSON.stringify(newUser));
      
      const profile = { name: authName, email: authEmail };
      setUser(profile);
      localStorage.setItem("sanjeevani_user", JSON.stringify(profile));
      handleNewConversation();
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = response.text || "I'm here listening, but I'm having a bit of trouble responding right now. Please try again or tell me more.";
      ...
      if (sentimentMatch) {
        ...
        setEvi(score);
        // Shift atmosphere with more granular color palette
        if (score < 15) setAtmosphericColor("#FF1744"); // Crisis/Deep Pain (Neon Red)
        else if (score < 30) setAtmosphericColor("#D500F9"); // Grief/Heavy (Electric Purple)
        else if (score < 45) setAtmosphericColor("#2979FF"); // Sadness/Melancholy (Cool Blue)
        else if (score < 60) setAtmosphericColor("#6E5BFF"); // Reflective/Exhausted (Deep Indigo)
        else if (score < 75) setAtmosphericColor("#00C2A8"); // Balanced/Seeking (Emerald)
        else if (score < 90) setAtmosphericColor("#00E676"); // Contentment/Growth (Mint)
        else setAtmosphericColor("#FFD600"); // Radiant Joy/Peak Peace (Amber Gold)
        
        // Extract AI generated insight and clean text
        const insightMatch = responseText.match(/\[INSIGHT:(.*?)\]/);
        let currentInsight = "";
        if (insightMatch) {
          currentInsight = insightMatch[1];
        }
        ...
        if (langMatch && user) {
          ...
          localStorage.setItem(`sanjeevani_lang_${user.email}`, newLang);
        }

        cleanText = responseText.replace(/\[SENTIMENT:\d+\]/, '').replace(/\[INSIGHT:.*?\]/, '').replace(/\[LANG:.*?\]/, '').trim();
        
        // Generate a proactive care insight after a delay using AI-provided text
        if (currentInsight) {
          setTimeout(() => {
            setInsight({ 
              text: currentInsight,
      ...
      // Split the response into chunks based on double newlines
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        // Wait briefly between bubbles to simulate natural typing
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "I apologize, but I encountered a small error in my systems. I'm still here for you. Could you please share that again?",
          timestamp: new Date().toISOString(),
        },
      ]);
    } finally {
  ...
  if (!user) {
    return (
      <div className="h-screen w-full flex items-center justify-center p-6 bg-obsidian relative overflow-hidden">
        {/* Animated background elements */}
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-emerald/5 blur-[120px] rounded-full animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-violet/5 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '2s' }} />

        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-md luxury-glass p-8 md:p-12 relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald to-violet" />
          ...
          <div className="mb-6 p-4 rounded-2xl bg-white/5 border border-white/10 text-center">
            <p className="text-[10px] text-cool-light uppercase tracking-widest font-bold mb-2">Pulse Sensor Status</p>
            <div className="flex justify-center items-center gap-2">
              <Heart size={14} className="text-red-400 animate-pulse" />
              <span className="text-xs font-mono text-soft-white">Ready for Synchronous Resonance</span>
            </div>
          </div>
          ...
          <form onSubmit={handleAuth} className="space-y-6">
            {!isLoginView && (
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-cool-light font-bold ml-2">Name</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-graphite/40 border border-slate-steel rounded-2xl py-4 pl-12 pr-4 text-sm text-soft-white focus:outline-none focus:border-emerald/50 transition-colors"
                  />
                </div>
              </div>
            )}
            ...
            {authError && (
              <motion.div 
                ...
                className="flex items-center gap-2 text-xs text-red-400 bg-red-400/10 p-3 rounded-xl border border-red-400/20"
              >
          ...
          <p className="mt-8 text-center text-[10px] text-cool-light/30 uppercase tracking-tighter">
            Syvon AI Labs • Mental Wellness Intelligence
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    <>
      {/* Audio Sanctuary Removed */}

      <div className="h-screen w-full flex p-6 gap-6 overflow-hidden bg-obsidian relative">
        <AnimatePresence>
          {insight && (
            <motion.div 
              ...
              className="fixed bottom-10 right-10 z-[100] luxury-glass p-6 pr-8 max-w-[280px] border-l-4 border-l-emerald shadow-2xl overflow-hidden"
            >
      ...
      {/* Sidebar */}
      <aside className="w-72 luxury-glass flex flex-col p-6 shrink-0 relative overflow-hidden z-20">
        ...
        <nav className="flex flex-col gap-3 flex-1 relative z-10 overflow-y-auto no-scrollbar pb-6">
          <div className="flex items-center justify-between mb-1 ml-3 px-2">
      ...
      <main className="flex-1 flex flex-col gap-6 overflow-hidden">
        {/* Header */}
        <header className="luxury-glass h-20 px-8 flex items-center justify-between shrink-0 shadow-xl">
...
function ChatView({ messages, scrollRef, isLoading, input, setInput, handleSend, user }: any) {
  return (
    <motion.section 
      key="chat"
    ...
    >
      <div 
        ref={scrollRef}
        className="flex-1 space-y-8 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-steel scrollbar-track-transparent scroll-smooth px-4"
      >
...
function JournalView({ moodHistory, setMoodHistory, user, ai }: any) {
  ...
  return (
    <motion.section 
      ...
      className="h-full flex flex-col p-8 luxury-glass overflow-hidden shadow-2xl"
    >
      <div className="grid grid-cols-1 md:grid-cols-5 gap-8 h-full overflow-hidden">
              ...
              return (
                <div key={entry.id} className={`luxury-card p-4 rounded-2xl border-l-4 shadow-sm transition-all hover:translate-x-1 ${colorClass}`}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button 
                      onClick={() => setMoodHistory(moodHistory.filter((m: any) => m.id !== entry.id))}
...
function GratitudeView({ list, setList }: any) {
  ...
  const addGratitude = () => {
    ...
    const colors = [
      ...
      "bg-sky-200/90 text-sky-900 border-sky-300",
      "bg-rose-200/90 text-rose-900 border-rose-300",
      "bg-emerald-200/90 text-emerald-900 border-emerald-300",
      "bg-violet-200/90 text-violet-900 border-violet-300"
    ];
  ...
  return (
    <motion.section 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="h-full flex flex-col p-8 luxury-glass overflow-hidden shadow-2xl relative"
    >
      {/* Background Pattern */}
          ...
          <div className="flex flex-wrap items-start justify-center gap-8 content-start">
            ...
            <AnimatePresence>
              {list.map((item: any) => (
                ...
                >
                  <button 
                    onClick={() => setList(list.filter((l: any) => l.id !== item.id))}
